...
merged_initial <- merge(filter_demo, filter_word, by=c("SUBJ", "VISIT"))
data <- merge(merged_initial, filter_utt, by=c("SUBJ", "VISIT"))
```


2f. Only using clinical measures from Visit 1
In order for our models to be useful, we want to minimize the need to actually test children as they develop. In other words, we would like to be able to understand and predict the children's linguistic development after only having tested them once. Therefore we need to make sure that our ADOS, MullenRaw(nonVerbalIQ), ExpressiveLangRaw(verbalIQ) and Socialization variables are reporting (for all visits) only the scores from visit 1.
...
data_final <- merge(data, data_visit1, by = c("SUBJ"))
```
...
write.csv(df_anon, file = "data_assignment1.csv") # writing a csv-file to the working directory
```
...
CHI1 <- df_anon %>%
  ...
  select(SUBJ) %>% 
  unique() #filtering out any subjects with an MLU across visit below 2.7

# Part 2
CHI2 <- df_anon %>% 
  filter(VISIT == "1") %>% 
  filter(CHI_MLU < 1.5) %>% 
  unique() # filtering out any subjects  with an MLU above 1.5 morphemes at first visit


# Part 3 - checking if all visits were done
CHI3 <- df_anon %>% 
  group_by(SUBJ) %>% 
  filter(n() == 6) %>%
  select(SUBJ) %>% 
...
# Part 3 - checking if there are any NAs in CHI_MLU
CHI3.1 <- df_anon %>%
...
tokens_df1 <- df_anon %>% 
  filter(VISIT == 6) %>% 
  arrange(tokens_CHI)
# subject 60 produced the most words on the 6th visit with a total of 1294

# part 2
tokens_df2 <- df_anon %>% 
  filter(VISIT == 1) %>% 
  arrange(tokens_CHI)
# subject 62 produced the least words on the 1st visit with a total of 0 words
```
...
# part 1
subset_ASD <- df_anon %>% 
  filter(Diagnosis == "ASD") %>% # filtering out people without ASD
  select(Diagnosis, CHI_MLU, tokens_CHI) # selecting the wanted columns

# part 2
subset_ASD <- df_anon %>% 
  select(Diagnosis, CHI_MLU, tokens_CHI, Diagnosis) # the second call of the recurring variable is ignored
```
...
df_anon <- df_anon %>% 
  group_by(VISIT) %>% 
  mutate(mean_visits = mean(tokens_CHI)) # taking the mean over-all words used pr. visit for all the trials including all children

# part 2
df_anon <- df_anon %>% 
  group_by(SUBJ) %>% 
  mutate(mean_overall = mean(tokens_CHI)) # taking the mean over-all words used and creating a new variable called mean_overall and adding it to the dataframe

# part 3
# Keeping the data allows us to look at the individual development. If we simply look at a mean we lose a lot of data-points. While this may be easier in creating a model, we can always get around the assumption of independence by creating a mixed effects model that accounts for individual intercepts for each subject by each trial.
```
